...
/** Represents a concrete action that is to be inserted into a plan. All parameters of the action refer to one global
  ...
  */
class Action(val abs:AbstractAction, val context:Context, val statements:List[TemporalStatement], val id:String, val parentAction:Option[Action])
  ...
  def reference = new ActionRef(name, abs.args.map(context.getGlobalVar(_)), id, parentAction.map(_.id))
...
object Action {
   ...
   * @param factory Variable factory used to create missing global variables
   ...
   */
  def apply(pb:AnmlProblem, ref:AbstractActionRef, factory:VariableFactory, parentAction:Option[Action]=None, contextOpt:Option[Context]=None) : Action = {
