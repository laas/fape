...
/**
 * incremental extension of the by-the-book Union Find structure 
 * 
...
public class UnionFind {
    ...
    static int incrementSize = 30;
    int count = 0;
    UF uf = new UF(incrementSize);
    
    private void AddComponent(int x){
        if(count - 1 == uf.id.length){
            //extend & copy
            UF newUf = new UF(uf.id.length);
            System.arraycopy(uf.id, 0, newUf.id, 0, uf.id.length);
            System.arraycopy(uf.rank, 0, newUf.rank, 0, uf.rank.length);
            newUf.count = uf.count + incrementSize;
            uf = newUf;
        } 
        in.put(x, count);
        out.put(count, x);
    ...
    /**
     * **************************************************************************
     * Compilation: javac UF.java Execution: java UF < input.txt Dependencies:
     ...
     * <em>union</em> and <em>find</em> operations, along with a
     * <em>connected</em> operation for determinig whether two sites in the same
     * component and a <em>count</em> operation that returns the total number of
     * components.
     * <p>
     * The union-find data type models connectivity among a set of <em>N</em>
     * sites, named 0 through <em>N</em> &ndash; 1. The <em>is-connected-to</em>
     * relation must be an
     ...
     * sites are in the same component if and only if they are connected. Both
     * sites and components are identified with integers between 0 and
     * <em>N</em> &ndash; 1. Initially, there are <em>N</em> components, with
     * each site in its own component. The <em>component identifier</em> of a
     * component (also known as the <em>root</em>, <em>canonical element</em>,
     * <em>leader</em>, or <em>set representative</em>) is one of the sites in
     * the component: two sites have the same component identifier if and only
     * if they are in the same component.
     * <ul>
     * <p>
     * <li><em>union</em>(<em>p</em>, <em>q</em>) adds a connection between the
     * two sites <em>p</em> and <em>q</em>. If <em>p</em> and <em>q</em> are in
     ...
     * </ul>
     * The component identifier of a component can change only when the
     * component itself changes during a call to
     * <em>union</em>&mdash;it cannot change during a call to <em>find</em>,
     * <em>connected</em>, or <em>count</em>.
     * <p>
     * This implementation uses weighted quick union by rank with path
     * compression by halving. Initializing a data structure with <em>N</em>
     * sites takes linear time. Afterwards, the <em>union</em>, <em>find</em>,
     * and <em>connected</em>
     * operations take logarithmic time (in the worst case) and the
     * <em>count</em> operation takes constant time. Moreover, the amortized
     * time per <em>union</em>, <em>find</em>, and <em>connected</em> operation
     * has inverse Ackermann complexity. For alternate implementations of the
     * same API, see {@link QuickUnionUF}, {@link QuickFindUF}, and
    ...
    public class UF {
         ...
         * integer in the pair represents some site; if the sites are in
         * different components, merge the two components and print the pair to
         * standard output.
         */
