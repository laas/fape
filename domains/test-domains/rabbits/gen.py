...
def gen(numAreas, numRabbitsPerArea, numAgents, numGoals, pbID = None):
    def randArea():
    ...
    if not pbID:
        pbID = '%dx%dx%dx%d' % (numAreas, numRabbitsPerArea, numAgents, numGoals)
...
for areas in [2,4,6,8,16]:
    for density in [1,2,4,8]:
        for goals in [2, 4, 8]:
            gen(areas, density, 3, goals)
